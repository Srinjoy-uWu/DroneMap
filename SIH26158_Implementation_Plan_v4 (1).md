...
| 7 | Added a note on using an AI coding assistant (e.g. Claude Code) per pipeline stage — this changes *how fast* you build, not *what* you build; the architecture and compute plan stay the same |
...
## 4. Building With AI Assistance

Since you're building most of this with AI coding help, the fastest path is an agentic coding tool that can work across the whole repo — running commands, installing dependencies, iterating on failures — rather than pasting code snippets back and forth. Claude Code is built for exactly this kind of multi-stage, multi-file pipeline build.

Two things AI assistance changes and doesn't change:
- **Changes:** how fast each stage's glue code, API wrappers, and integration scripts get written
- **Doesn't change:** VRAM ceilings, model licensing, dataset domain gaps, or the fact that a 3DGS training run still takes however long it takes on a T4. AI can write the training script in minutes; it can't make the GPU faster.

Treat each pipeline stage below as a self-contained task you can hand to an AI coding session with a clear spec (input format, output format, which repo to use) — that decomposition is exactly why the stage-by-stage structure below matters.
