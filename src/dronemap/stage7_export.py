...
def _write_laz(
    ...
    """Write dense cloud as LAZ with the run's CRS embedded."""
    try:
        import laspy
        import pyproj
...
def _write_accuracy_report(ws: "RunWorkspace", out_path: Path) -> dict:
...
def _write_html_report(ws: "RunWorkspace", out_path: Path, report: dict, gsd: float, crs: str) -> None:
...
<body>
  <div class="container">
    ...
    <div class="card">
      ...
      <div class="alert-box">
        <strong>Accuracy &amp; Geodetic Notice:</strong> {accuracy_stmt}
      </div>
    </div>
...
def run(ws: "RunWorkspace", config: "Config", tools: "ToolRegistry", ctx: "_StageContext") -> None:
    ...
    if cfg.point_cloud_laz and points is not None and is_georeferenced:
        ...
        try:
            _write_laz(points, colors, laz_path, crs, cfg.cloud_voxel_size)
    ...
    try:
        report = _write_accuracy_report(ws, report_path)
        _write_html_report(ws, html_report_path, report, gsd, crs)
